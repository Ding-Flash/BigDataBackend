...
class Parse:
    ...
    def __init__(self, path, all_nodes=True):
    ...
    def build_tree(self):
        ...
        for root in tqdm(self.call):
            ...
            deep_call(root, self.trees[root])
    ...
    def get_func_feature(self):
        self.build_tree()
        func_feature = self.func_feature.to_dict()

        def merge(k, v):
            v['name'] = k
            v['total'] = int(v['count'] * v['mean'])
            return v
        
        res = [merge(k, v) for k, v in func_feature.items()]
        return res
    
    def get_call_tree(self):
        self.build_tree()
        records = util.hash_tree(self.call, self.nodes)
        return {
            "records": records,
            "trees": self.trees
        }
    ...
    def get_time_line(self):
        self.build_tree()
        self.df['begin'] = self.df['begin'] - self.df['begin'].min()
        return self.df